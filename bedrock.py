...
class TableColumnRelationship(BaseModel):
	...
	table_name: str = Field(
		description="The actual table name (resolved from variables if needed and also resolve with cte_names to get actual table name only)")
	column_name: str = Field(description="The specific column name, or * for table-level operations")
	relationship_types: str = Field(
		description="Comma-separated list of relationship types: USED_AS_SELECT, USED_AS_JOIN, USED_AS_FILTER, USED_AS_GROUP_BY, USED_AS_ORDER_BY, USED_AS_HAVING, PROCEDURE_CREATES_TABLE, USED_AS_UPDATE, USES_TABLE, USED_AS_INSERT, USED_AS_MERGE, USED_AS_DELETE")
...
class StoredProcedureAnalysis(BaseModel):
	...
	relationships: List[TableColumnRelationship] = Field(
		description="List of ALL table-column relationships found in the procedure")
...
def extract_temp_tables(sp_definition: str) -> List[str]:
	...
	pattern = re.compile(
		r"CREATE\s+(?:OR\s+REPLACE\s+)?(?:TEMPORARY|TEMP)\s+TABLE\s+(?:IDENTIFIER\s*\(\s*:?(\w+)\s*\)|([A-Z_][A-Z0-9_\.]*)|([A-Z_][A-Z0-9_\.]*\s*\|\|\s*.*?))",
		re.IGNORECASE)
...
def save_relationships_csv(all_analyses: List[StoredProcedureAnalysis], filename: str):
	...
	logger.info(f"CSV saved: {filename} ({total_relationships} total consolidated relationships)")
...
def analyze_procedures_parallel(procedures: List[Dict], max_workers: int = 4) -> List[StoredProcedureAnalysis]:
		...
		for future in as_completed(future_to_procedure):
			...
			try:
				...
				if result:
					...
					logger.info(
						f"Completed {completed_count}/{len(procedures)}: {procedure['procedure_name']} - {len(result.relationships)} relationships")
				else:
					logger.warning(
						f"Completed {completed_count}/{len(procedures)}: {procedure['procedure_name']} - FAILED")
			except Exception as e:
				logger.error(f"Exception in procedure {procedure['procedure_name']}: {e}")

	logger.info(f"Parallel analysis complete: {len(results)} successful, {len(procedures) - len(results)} failed")
	return results


# --- 7. Main Execution Functions ---
def analyze_all_procedures(sf_env: str = 'prod', max_workers: int = 4, output_file: str = "sp_analysis_results.csv"):
	...
	try:
		...
		# Fetch procedures from Snowflake
		...
		results = analyze_procedures_parallel(procedures, max_workers)
		...
		save_relationships_csv(results, output_file)
		...
		logger.info(f"Average time per procedure: {elapsed_time / len(procedures):.2f} seconds")
...
if __name__ == "__main__":
	# Configuration
	SF_ENVIRONMENT = 'prod'  # Change to 'dev' or 'stage' as needed
